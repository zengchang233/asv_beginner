...
class SpeakerEmbNet(nn.Module):#定义resnet
    def __init__(self, opts):#block类型，embedding大小，分类数，maigin大小
        ...
        #  block_layers = opts['block_layers']
        ...
        #  self.conv1 = nn.Conv2d(input_dim, hidden_dim[0], kernel_size=5, stride=2, padding=2, bias=False)
    ...
    def forward(self, x):
        ...
        #  x = self.avg_pool(x)
        #  x = x.view(x.size(0), -1)
        #  x = self.fc(x) # output after batch normalization
        #  return x
